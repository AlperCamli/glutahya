  ...
  return (
    <section id="why" style={{ background: `radial-gradient(ellipse 60% 50% at 80% 20%, oklch(84% 0.07 162 / 0.22) 0%, transparent 60%), radial-gradient(ellipse 40% 40% at 10% 80%, oklch(78% 0.08 292 / 0.18) 0%, transparent 60%), #f2f2f8`, padding: 'clamp(80px,10vw,130px) clamp(24px,5vw,80px)', position: 'relative', overflow: 'hidden' }}>
      <div ref={ref} style={{ maxWidth: '1200px', margin: '0 auto', opacity: visible ? 1 : 0, transform: visible ? 'none' : 'translateY(40px)', transition: 'all 0.9s ease' }}>
        ...
        {/* Before / After */}
        <div style={{ background: 'rgba(255,255,255,0.75)', borderRadius: '24px', padding: '48px', display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: '40px', alignItems: 'center', border: '1px solid rgba(255,255,255,0.95)', backdropFilter: 'blur(20px)', boxShadow: '0 8px 40px rgba(0,0,0,0.06)' }}>
          <div>
            ...
            {['Tight after showering', 'Grey, flat appearance', 'Rough to the touch', 'Feels uncomfortable'].map(t => (
              <div key={t} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                <span style={{ width: '16px', height: '1px', background: 'rgba(0,0,0,0.15)' }}></span>
                <span style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '14px', color: T.muted }}>{t}</span>
              </div>
            ))}
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
            <div style={{ width: '1px', height: '40px', background: `linear-gradient(to bottom, transparent, ${T.lavender})` }}></div>
            <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: `linear-gradient(135deg, ${T.lavender}, ${T.blue})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', color: '#fff', boxShadow: `0 8px 24px oklch(78% 0.08 292 / 0.3)` }}>→</div>
            <div style={{ width: '1px', height: '40px', background: `linear-gradient(to top, transparent, ${T.mint})` }}></div>
          </div>
          <div>
            ...
            {['Soft, hydrated all day', 'Radiant, luminous glow', 'Visibly smooth texture', 'Confident in your skin'].map(t => (
              <div key={t} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                <span style={{ width: '16px', height: '1px', background: `${T.mint}` }}></span>
  ...
  return (
    <section id="movement" style={{ background: `radial-gradient(ellipse 50% 60% at 90% 30%, oklch(86% 0.06 352 / 0.25) 0%, transparent 60%), radial-gradient(ellipse 45% 55% at 5% 70%, oklch(82% 0.07 228 / 0.2) 0%, transparent 60%), #eeeef5`, padding: 'clamp(80px,10vw,130px) clamp(24px,5vw,80px)', overflow: 'hidden', position: 'relative' }}>
      <div ref={ref} style={{ maxWidth: '1200px', margin: '0 auto', opacity: visible ? 1 : 0, transform: visible ? 'none' : 'translateY(40px)', transition: 'all 0.9s ease' }}>
        ...
        {/* Quote cards */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '18px', marginBottom: '52px' }}>
          {quotes.map((q, i) => (
            <div key={i}
...
const PRODUCTS = [
  { id: 'gold', name: 'Flawless Bright', sub: 'Anti-Spot & Brightening', color: T.peach, image: _r('gold', 'uploads/sar\u0131.webp'), concern: 'Dark spots & uneven tone', benefit: 'Visibly reduces dark spots in 5 days', ingredients: ['GlutaGlow Complex', 'Hyaluronic Acid', 'Pro-Retinol'], moment: 'Morning — before getting dressed', tag: 'Anti-Spot' },
  { id: 'pink', name: 'Dewy Radiance', sub: 'Glow & Hydration Booster', color: T.blush, image: _r('pink', 'uploads/pembe.webp'), concern: 'Dullness & loss of luminosity', benefit: 'Luminous skin in 5 days', ingredients: ['GlutaGlow Complex', 'Hyaluronic Acid', 'Niacinamide'], moment: 'Everyday glow — morning or night', tag: 'Glow Boost' },
  { id: 'blue', name: 'Smoothing Perfector', sub: 'Texture & Renewal Formula', color: T.blue, image: _r('blue', 'uploads/mavi.webp'), concern: 'Rough texture & body acne', benefit: 'Renewed, smooth skin in 5 days', ingredients: ['GlutaGlow Complex', 'Hyaluronic Acid', 'AHA · BHA · PHA'], moment: 'Post-shower — after exfoliating', tag: 'Smooth + Renew' },
  { id: 'red', name: 'Pro Age Restore', sub: 'Firming & Elasticity Formula', color: T.blush, image: _r('red', 'uploads/k\u0131rm\u0131z\u0131.webp'), concern: 'Loss of firmness & elasticity', benefit: 'Firmer, more elastic skin in 5 days', ingredients: ['GlutaGlow Complex', 'Hyaluronic Acid', 'Collagen Booster'], moment: 'Evening — night ritual', tag: 'Firm & Restore' },
  { id: 'orange', name: 'SPF 50 Shield', sub: 'Daily Sun Protection + Glow', color: T.peach, image: _r('orange', 'uploads/turuncu.webp'), concern: 'Sun damage & UV-induced spots', benefit: 'UVA + UVB protection with glow', ingredients: ['GlutaGlow Complex', 'Hyaluronic Acid', 'UVA/UVB Filters SPF50'], moment: 'Morning — before going outdoors', tag: 'Daily Protection' },
];
  ...
  return (
    <div onClick={() => setExpanded(!expanded)} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      ...
      <div style={{ display: 'flex', gap: '20px', alignItems: 'flex-start' }}>
        ...
        <div style={{ color: product.color, fontSize: '18px', transition: 'transform 0.3s', transform: expanded ? 'rotate(90deg)' : 'none' }}>›</div>
      </div>
      {expanded && (
    ...
    <section style={{ background: `radial-gradient(ellipse 55% 55% at 10% 30%, oklch(82% 0.07 228 / 0.22) 0%, transparent 60%), radial-gradient(ellipse 40% 40% at 90% 70%, oklch(84% 0.07 162 / 0.18) 0%, transparent 60%), #eef0f5`, padding: 'clamp(80px,10vw,130px) clamp(24px,5vw,80px)', overflow: 'hidden', position: 'relative' }}>
      <div ref={ref} style={{ maxWidth: '1200px', margin: '0 auto', opacity: visible ? 1 : 0, transform: visible ? 'none' : 'translateY(40px)', transition: 'all 0.9s ease' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', marginBottom: '40px' }}>
          {COMBOS.map((combo, i) => (
            <button key={i} onClick={() => setActive(i)} style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '14px', fontWeight: i === active ? 600 : 400, background: i === active ? 'rgba(255,255,255,0.9)' : 'rgba(255,255,255,0.5)', color: i === active ? T.text : T.body, border: `1px solid ${i === active ? 'rgba(255,255,255,0.95)' : 'rgba(255,255,255,0.7)'}`, borderRadius: '100px', padding: '12px 28px', cursor: 'pointer', transition: 'all 0.3s ease', backdropFilter: 'blur(12px)', boxShadow: i === active ? '0 4px 20px rgba(0,0,0,0.08)' : 'none' }}>{combo.title}</button>
          ))}
        </div>

        <div style={{ background: 'rgba(255,255,255,0.72)', backdropFilter: 'blur(24px)', border: '1px solid rgba(255,255,255,0.95)', borderRadius: '28px', padding: '48px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '48px', alignItems: 'center', boxShadow: '0 8px 48px rgba(0,0,0,0.06)', transition: 'all 0.5s ease' }}>
          <div>
